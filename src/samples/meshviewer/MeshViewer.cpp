...
MeshViewer::MeshViewer() {
    ...
    obj_entity_->scale(glm::vec3(.8f, .8f, .8f));
    // light
    for (int i = 0; i < N_LIGHT; ++i) {
        ...
        light_[i] = new game::Light(glm::vec3(2.f / N_LIGHT));
        game::lights.push_back(light_[i]);
        light_entity_[i]->addChild("light", light_[i]);
        light_entity_[i]->scale(glm::vec3(.05f, .05f, .05f));
        light_entity_[i]->translate(glm::vec3(.3f, .4f, .8f));
        light_entity_[i]->rotate(360 / N_LIGHT * i, glm::vec3(0, 0, 1));
    }
